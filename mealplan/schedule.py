...
# Collagen - workout days in smoothie, non-workout days in matcha
WORKOUT_DAYS = {"wednesday", "friday", "sunday"}
MATCHA_DAYS = {"monday", "tuesday", "thursday", "saturday"}
...
# Monday-Thursday (Simple Prep) schedule
SIMPLE_PREP = {
    "monday": [
        ...
        Meal("3:00pm", "Matcha Latte", "1 cup oat milk + 1 tsp matcha + 1 scoop collagen"),
...
# Friday-Sunday (Indulgent) schedule
INDULGENT = {
    "friday": [
...
def get_meals_for_day(day_name: str) -> list[Meal]:
    """Get the meal schedule for a given day of the week."""
    day_lower = day_name.lower()
    if day_lower in SIMPLE_PREP:
        return SIMPLE_PREP[day_lower]
    elif day_lower in INDULGENT:
        return INDULGENT[day_lower]
    else:
        raise ValueError(f"Unknown day: {day_name}")


def is_workout_day(day_name: str) -> bool:
    """Check if this is a workout day (Wed/Fri/Sun)."""
    return day_name.lower() in WORKOUT_DAYS
...
def is_iron_day(target_date: date, overrides_dir: Path) -> bool:
    ...
    Uses a deterministic pattern: even days (since start) are iron days.
    ...
    # Deterministic alternation: days since start date, mod 2
